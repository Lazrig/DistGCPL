 ...
 * @param <T>
 */
public class GenWssBFs <T> extends network.Server{
    ...
    public GenWssBFs(int port, Mode mode, int numOfTasks, boolean[][][] bin_a, boolean[][] Ws_a, boolean[] threshold_a, int len_b, ArrayList<String> PartyA_IDs) {
        this.port = port;
        this.mode = mode;
        this.bin_a = bin_a;
        this.Ws_a = Ws_a;
    ...
    public void implement() {
        ...
        try {
           ...
           // PatientLinkageWssBFGadget.resetBar();
            ...
            Object[] result = pool.runGadget(new PatientLinkageWssBFGadget(), inputs);
            ...
            for(int i = 0; i < d2.length; i++){
                for(int j = 0; j < d2[i].length; j++){
                    ...
                    z3[i][j] = Util.toInt(z2[i][j]); //2*Intersection AB
                }
            }
            os.flush();
             ...
             for(int i = 0; i < d3.length; i++){
                for(int j = 0; j < d3[i].length; j++){
                    ...
                    z3_2[i][j] = Util.toInt(z2_2[i][j]); //Union AB
                }
            }
            os.flush(); 
            //end
