 ...
 * @param <T>
 */
public class EnvWssBFsWith1scntAndChkDCi <T> extends network.Client{
    String addr;
    int port;
    Mode mode;
    boolean[][][] bin_b;
    ...
    public EnvWssBFsWith1scntAndChkDCi(String addr, int port, Mode mode, int numOfTasks, boolean[][][] bin_b,boolean[][][] BFs1sCnt_b, boolean[][] Ws_b, boolean[] threshold_b,boolean[][] chkDCi, int len_a, ArrayList<String> PartyB_IDs) {
        this.addr = addr;
        this.port = port;
        this.mode = mode;
        this.bin_b = bin_b;
    ...
    public void implement() {
        ...
        try {
            ...
            for(int i = 0; i < this.numOfTasks; i++){
                PatientLinkage4GadgetWsBF1sCntInputs<T> tmp0 = new PatientLinkage4GadgetWsBF1sCntInputs<>(Arrays.copyOfRange(bin_a, Range0[i][0], Range0[i][1]), Arrays.copyOfRange(BFs1sCnt_a, Range0[i][0], Range0[i][1]) , Ws_a, threshold_a,
                        Arrays.copyOfRange(achkDCi, Range0[i][0], Range0[i][1]), eva, "Alice", i);
                PatientLinkage4GadgetWsBF1sCntInputs<T> tmp1 = new PatientLinkage4GadgetWsBF1sCntInputs<>(this.bin_b,this.BFs1sCnt_b, Ws_b, threshold_b,
